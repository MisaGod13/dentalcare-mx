-- Función RPC segura para crear usuarios reales de pacientes en Supabase Auth
...
BEGIN
  ...
  FROM profiles 
  WHERE patient_id = patient_id AND role = 'patient';
...
COMMENT ON FUNCTION create_real_patient_auth IS 'Crea un usuario real de paciente en auth.users y profiles para login real (versión segura)';
