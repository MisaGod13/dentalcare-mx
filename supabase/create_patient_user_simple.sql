-- Función RPC simple para crear usuarios de pacientes (sin verificación de rol)
-- Esta función debe ejecutarse desde el SQL Editor de Supabase

-- Función que crea un usuario real en auth.users y profiles
CREATE OR REPLACE FUNCTION create_patient_user_simple(
  patient_email TEXT,
...
GRANT EXECUTE ON FUNCTION create_patient_user_simple(TEXT, TEXT, TEXT, UUID, UUID) TO authenticated;

-- Comentario de la función
COMMENT ON FUNCTION create_patient_user_simple IS 'Crea un usuario de paciente sin verificación de rol del dentista';
