-- Función RPC corregida para crear usuarios reales de pacientes
-- Esta función debe ejecutarse desde el SQL Editor de Supabase

-- Función que crea un usuario real en auth.users (versión simplificada)
...
BEGIN
  ...
  -- Crear el usuario en auth.users (solo columnas esenciales)
...
COMMENT ON FUNCTION create_real_patient_user IS 'Crea un usuario real de paciente en auth.users y profiles (versión simplificada)';
