-- Función RPC corregida para crear usuarios reales de pacientes en Supabase Auth
-- Esta función debe ejecutarse desde el SQL Editor de Supabase

-- Función que crea un usuario real en auth.users y profiles de forma segura
...
BEGIN
  ...
  FROM profiles 
  WHERE profiles.patient_id = patient_id AND profiles.role = 'patient';

  -- Si ya existe un perfil, devolver error
  ...
  IF EXISTS (
    ...
    WHERE auth.users.email = patient_email
  ) THEN
    result := jsonb_build_object(
      'success', false,
  ...
  INSERT INTO auth.users (
    ...
    (SELECT auth.instances.id FROM auth.instances LIMIT 1),
    new_user_id,
    'authenticated',
    'authenticated',
    patient_email,
...
COMMENT ON FUNCTION create_real_patient_auth IS 'Crea un usuario real de paciente en auth.users y profiles para login real (versión corregida)';
